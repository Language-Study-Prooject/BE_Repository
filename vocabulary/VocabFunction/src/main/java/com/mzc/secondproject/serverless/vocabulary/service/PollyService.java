package com.mzc.secondproject.serverless.vocabulary.service;
...
public class PollyService {
    ...
    private static final String bucketName = System.getenv("VOCAB_BUCKET_NAME");

    /**
     * 단어 ID 기반으로 음성 합성 (캐시 지원)
     * S3에 파일이 있으면 바로 URL 반환, 없으면 Polly 변환 후 저장
     */
    public VoiceSynthesisResult synthesizeSpeechForWord(String wordId, String text, String voice) {
        String s3Key = generateS3Key(wordId, voice);
    ...
    /**
     * 단어 ID와 음성 타입으로 S3 키 생성
     */
    public String generateS3Key(String wordId, String voice) {
        String voiceSuffix = "MALE".equalsIgnoreCase(voice) ? "male" : "female";
        return "vocab/voice/" + wordId + "_" + voiceSuffix + ".mp3";
    }
